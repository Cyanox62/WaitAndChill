	...
	public class EventHandler
	{
		...
		public void RunWhenPlayersWait()
		{
			Timing.CallDelayed(1f, () =>
			{
				...
				{
					Room.Doors.First(d => d.name == "Unsecured Pryable GateDoor").ServerChangeLock(DoorLockReason.AdminCommand, true);
				}
		...
		public void RunWhenRoundStarts()
		{
			...
			if (Room != null)
			{
				foreach (DoorVariant door in Room.Doors) door.ServerChangeLock(DoorLockReason.AdminCommand, false);
		...
		public void RunWhenPlayerJoins(JoinedEventArgs JoinEv)
		{
			Timing.CallDelayed(1f, () =>
			{
				if (!Round.IsStarted && (GameCore.RoundStart.singleton == null || GameCore.RoundStart.singleton.NetworkTimer > 1 || GameCore.RoundStart.singleton.NetworkTimer == -2))
				{
		...
		{
			if (!Round.IsStarted && (GameCore.RoundStart.singleton == null || GameCore.RoundStart.singleton.NetworkTimer > 1 || GameCore.RoundStart.singleton.NetworkTimer == -2) && SpawnEv.RoleType == RoleType.Tutorial)
			{
